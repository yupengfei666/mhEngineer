...
<script>
	...
	export default {
		...
		methods: {
			...
			ok(){
				uni.navigateTo({
					url: "../tab3/old-agent?beginDate=" + this.startDate + " " + this.startTime + ":00" +
			...
			getClassify(){
				uni.request({
					url: this.url + "WeChat/GetClassify",
					data: {
						DeviceID: 3,
					},
					method: 'GET',
					...
					success: (res) => {
						...
						}else
						{
							for (var i = 0, lenI = res.data.length; i < lenI; ++i) {
								this.classifyList.push(res.data[i].机构);
							}
							this.classifyList.unshift("全部");
						}
					},
					fail: () => {
						uni.showToast({
